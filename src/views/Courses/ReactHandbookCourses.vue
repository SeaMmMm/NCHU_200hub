<template>
  <CourseTemplate :contents="contents" where="react_handbook_course" />
</template>
...
export default {
  ...
  methods: {
    async getcartoonUrl() {
      ...
      this.index = index;
      const query = `{
        reactHookHandbookCollection(where: {AND: [{index: ${index} }]}) {
          items {
            title
            index
            description
      ...
      try {
        ...
        return response.data.reactHookHandbookCollection.items;
      } catch (error) {
