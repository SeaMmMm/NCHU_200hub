<template>
  <CourseTemplate :contents="contents" where="swiftui_v14_03_course" />
</template>
...
export default {
  ...
  methods: {
    async getcartoonUrl() {
      ...
      this.index = index;
      const query = `{
        swiftUiV14P3Collection(where: {AND: [{index: ${index} }]}) {
          items {
            title
            index
            description
      ...
      try {
        ...
        return response.data.swiftUiV14P3Collection.items;
      } catch (error) {
